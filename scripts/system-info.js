...
function adjustForDecimals(value, decimals=(new web3.BigNumber(18))) {
...
module.exports = async function(callback) {
  ...
  let cst, cstRegistry, cstFrozen, cstStorageName, cstLedgerName, cstName,
    ...
    contributionMinimum, vestingCount, cstAvailable, cstTotalInCirculation, decimals, cstVersion, defaultLimitEth,
    registryVersion, registryOwner, storageOwner, ledgerOwner, cstOwner;
  ...
  try {
    ...
    registryVersion = "1"; // this property wasnt introduced in the initial contracts
  }
  ...
  if (cstAddress === NULL_ADDRESS) {
    ...
    try {
      ...
      console.error("Encountered error getting token details", err);
    }
  ...
  try {
    ...
    if (storageAddress === NULL_ADDRESS) {
      ...
      storageOwner = await storage.owner();
    }
    ...
    if (ledgerAddress === NULL_ADDRESS) {
      ...
      ledgerOwner = await ledger.owner();
    }
  } catch (err) {
    console.error("Encountered error getting token details", err);
  }
...
Cardstack Token (${cst.address}):
  ...
  Frozen Accounts:`);
    ...
    console.error("Error encountered displaying system info:", err);
  }
